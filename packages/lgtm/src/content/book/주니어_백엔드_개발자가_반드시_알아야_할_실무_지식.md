...
- [5장. 비동기 연동, 언제 어떻게 써야 할까?](#5장-비동기-연동-언제-어떻게-써야-할까)
  ...
  - [메시징](#메시징)
    ...
    - [메시지 소비 측 고려 사항](#메시지-소비-측-고려-사항)
    - [트랜잭션 아웃박스 패턴](#트랜잭션-아웃박스-패턴)
    - [아웃박스 테이블 구조](#아웃박스-테이블-구조)
  - [CDC(Change Data Capture)](#cdcchange-data-capture)
    - [CDC와 데이터 위치](#cdc와-데이터-위치)
    - [CDC가 유용할 때](#cdc가-유용할-때)
...
- [10장. 모르면 답답해지는 네트워크 기초](#10장-모르면-답답해지는-네트워크-기초)
  - [VPN](#vpn)
  - [TCP, UDP, QUIC](#tcp-udp-quic)
- [11장. 자주 쓰는 서버 구조와 설계 패턴](#11장-자주-쓰는-서버-구조와-설계-패턴)
  - [계층형 아키텍처(Layered Architecture)](#계층형-아키텍처layered-architecture)
    - [흩어지는 도메인 로직](#흩어지는-도메인-로직)
  ...
  - [CQRS 패턴 (Command Query Responsibility Segregation)](#cqrs-패턴-command-query-responsibility-segregation)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-1.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-2.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-3.png)
...
익셉션을 전파해도 소용없기 때문이다.별도 스레드로 실행되는 코드는 내부에서 연동 과정에서 발생한 오류를 직접 처리해야 한다.
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-4.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-5.png)

시스템 A가 메시지를 발행하면 시스템 B에서도 소비를 하고 시스템 C에서도 소비를 하는 구조를 말하는 것 같다.
...
두 번째 방법은 재시도하는 것이다. 일시적인 네트워크 불안정과 같은 오류는
재시도를 통해 해결될 수 있다.
하지만 메시지 전송을 재시도하는 과정에서 중복된 메시지가 전송될 수 있다.
실제로는 전송에 성공했는데 일시적인 네트워크 오류로 전송에 실패한 것으로 인지하고
재시도를 할 수 있기 떄문이다.
메시징 시스템이 중복 수신을 방지하는 기능을 제공하지 않으면 메시지 소비자가 중복 메시지를 알맞게 처리해야 한다.

메시지마다 고유 식별자를 사용하면 메시지 소비자가 중복 메시지 여부를 판단하는데 도움이 된다.

메세지 생산자는 DB 트랜잭션과의 연동도 고려해야 한다. DB 트랜잭션에 실패했는데 메시지가 발송되면 잘못된 데이터가 전달될 수 있기 때문이다.

```mermaid
---
title: 메시지를 전송한 뒤 DB의 트랜잭션이 롤백되면 잘못된 메시지가 전송될 수 있다.
---
...
sequenceDiagram
  ...
  participant DB
  participant messageQueue as 메시지 큐
  participant notificationService as 알림 서비스

  user ->>+ orderService: 요청
  orderService ->> DB: 트랜잭션 시작
  activate DB
  deactivate DB

  orderService ->> DB: DB 변경
    activate DB
    deactivate DB

    orderService ->>+ messageQueue: 메시지 전송
    messageQueue -->>- notificationService: 메시지 전파
    activate notificationService

    orderService ->>+ DB: DB 변경
    DB -->>- orderService: 변경 실패

    orderService ->> DB: 트랜잭션 롤백
    activate DB
    deactivate DB

    orderService -->>- user: 에러 응답

    notificationService -->> user: 주문 안내 푸시 발송
    deactivate notificationService
```

트랜잭션 롤백(8)했지만 알림 서비스는 고객에게 주문이 완료됐다는 푸시를 보낸다.
고객은 주문이 실패했다는 에러 화면을 봤지만 잠시 뒤에 주문이 완료됐다는 푸시를 받게 된다.

```mermaid
---
title: 트랜잭션이 완료된 뒤에 메시지를 전송한다.
---
...
sequenceDiagram
  ...
  orderService ->> DB: DB 변경
    ...
    activate DB
    deactivate DB
    orderService ->> DB: 트랜잭션 커밋
    activate DB
    deactivate DB

    orderService ->>+ messageQueue: 메시지 전송
    orderService -->>- user: 성공응답
    messageQueue -->>- notificationService: 메시지 전파
    activate notificationService
...
- 메세지 생산자가 같은 데이터를 가진 메시지를 메시징 시스템에 두 번 전송
- 소비자가 메시지를 처리하는 과정에서 오류가 발생하여 메시지 재수신

메시지 생산자가 동일한 데이터를 가진 메시지를 두 번 이상 메시징 시스템에 전송하면 메시지 수신자는 중복해서 메시지를 처리하는 것과 같다. 수신자 입장에서 동일 데이터를 가진 중복 메시지를 처리하는 방법은 메시지에 고유한 ID를 부여해서 이미 처리했는지 여부를 추적하는 것이다.
이를 통해 이미 처리한 메시지는 다시 처리하지 않고 무시할 수 있다.

메시지를 처리했는지 여부는 DB 테이블에 기록하거나 메모리에 집합으로 관리하면 된다.
메모리로 관리할 때는 OOM 에러가 발생하는 것을 막기 위해 일정 개수의 메시지 ID만 유지한다.

### 트랜잭션 아웃박스 패턴

메시지 데이터 자체가 유실되지 않도록 보장하는 방법은 먼저 해당 데이터를 DB에 안전하게 저장해두는 것이다.
그 뒤, 저장된 메시지를 읽어 메시징 시스템에 전송하면 된다.
이처럼 메시지 데이터를 DB에 보관하는 방식이 바로 트랜잭션 아웃박스 패턴(Transaction Outbox Pattern)의 핵심이다.

트랜잭션 아웃박스 패턴은 하나의 DB 트랜잭션 내에서 다음 2가지 작업을 수행한다.

- 실제 비즈니스 로직에 필요한 DB 변경 작업
- 메시징 시스템에 전송할 메시지 데이터를 아웃박스 테이블에 저장하는 작업

아웃박스 테이블에 쌓인 메시지 데이터는 별도의 메시지 중계 프로세스가 주기적으로 읽어서 메시징 시스템에 전송한다.

```mermaid
---
title: 트랜잭션 아웃박스 패턴의 동작과정
---

sequenceDiagram
  autonumber
  participant systemA as 시스템 A
  participant DB
  participant messageMiddleMan as 메시지 중계
  participant messageQueue as 메시지 큐
    ...
    systemA ->> DB: 아웃박스 테이블에 메시지 저장
    systemA ->> DB: 트랜잭션 커밋
    deactivate DB

    loop
        messageMiddleMan ->> DB: 메시지 조회
        activate DB
        messageMiddleMan -->> messageQueue: 메시지 전송
        messageMiddleMan ->> DB: 전송 완료 표기
        deactivate DB
    end
```
...
아웃박스 테이블에 발송 상태 컬럼은 다음 3가지 상태를 가진다.

- PENDING: 메시지 전송 대기 상태
- SENT: 메시지 전송 완료 상태
- FAILED: 메시지 전송 실패 상태
...
erDiagram
    ...
    outbox {
        bigInt outboxId "PK"
        varchar messageId "메시지 ID (고유키)"
        varchar messageType "메시지 타입"
        clob payload "메시지 데이터"
        varchar status "메시지 전송 상태 (WAITING, DONE, FAILED)"
        timestamp failCount "실패 횟수"
        timestamp occuredAt "발생 시각"
        timestamp processedAt "처리 시각"
        timestamp failedAt "실패 시각"
    }
```

## CDC(Change Data Capture)

변경된 데이터를 추적하고 판별해서 변경된 데이터로 작업을 수행할 수 있도록 하는 기술이다.

DBMS는 데이터가 변경되면 그 변경 내용을 통지하는 기능을 제공한다.
CDC는 이 기능을 활용해서 구현한다.
CDC 패턴에서는 3개의 구성 요소가 등장한다.

```mermaid
flowchart LR
    A[DBMS]  -- "`변경분 통지`" --> B[CDC 처리기]
    B -->|변경된 데이터| C[메시징 시스템]
    C -->|메시지 전송| D[대상 시스템]
```

커밋된 데이터만 변경된 순서에 맞게 전달한다. CDC 처리기에는 롤백된 데이터가 전달되지 않는다. 또한 잘못된 순서로 데이터가 전달되는 일도 없다.
...
예를 들어, 회원 시스템의 회원 데이터를 컨텐츠 시스템의 회원 테이블에 동기화하는 상황을 생각해보자. 이때 두 시스템의 회원 데이터는 거의 1대 1의 관계를 갖는다.

회원 시스템의 회원 테이블의 이름 컬럼을 변경하면 컨텐츠 시스템의 회원 테이블에도 이름 컬럼을 변경되는 식이다.

```mermaid
---
title: 변경 데이터를 그대로 전파하는 경우
---
...
flowchart LR
    ...
    subgraph DB
    member -- "member 테이블<br/>name 변경" --> B
    end
    ...
    B --member 테이블<br/>name 변경--> member2
```

변경 데이터를 그대로 전파하지 않고 변환해서 전파해야 할 때도 있다.
예를 들어, 주문 시스템과 통지 시스템을 연동한다고 하자.
이때 CDC 처리기는 새로운 주문 데이터가 생성되거나 주문 상태가 '배송 중'으로 변경되면 통지 시스템에 주문 데이터를 모두 전송할 필요가 없다.

단지 주문 ID와 주문 상태만 전송하면 된다.

```mermaid
---
title: 변경 데이터를 변환해서 전파하는 경우
---
flowchart LR
    order[주문 DB] -- "변경분 통지" --> B[CDC 처리기]
    B --"(주문 ID, 상태)"--> notifySystem[통지 시스템]
```

목적에 따라 CDC 처리기는 DB, 메시징 시스템, API 등 다양한 대상에 데이터를 전파할 수 있다. 메시징 시스템에 데이털르 전파하면 여러 시스템에 변경된 데이터를 전달할 수 있어 확장에 유리하다.

```mermaid
---
title: 메시징 시스템에 변경 데이터를 전파하면 확장에 유리하다.
---

flowchart LR
  DB -- "변경분 통지" --> B[CDC 처리기]
  B -- "변경된 데이터" --> messageQueue[메시징 시스템]
  messageQueue --> systemA[시스템 A]
  messageQueue --> systemB[시스템 B]
```
...
> 데이터의 변경 분을 통해서 의미를 도출할 뿐이다.
> 예를 들어, '회원 암호 초기화함'이벤트는 암호를 초기화했다는 것을 분명하게 나타내지만, '회원 테이블 데이터 변경: 이전암호, 이후 암호' 데이터 변경분은 암호가 초기화됐음을 드러내지 않는다.
> 단지 암호 컬럼 값이 변경됐다는 것만 알 수 있다.
...
시스템이 복잡해서 연동 코드를 넣기 부담스러울 때 CDC를 유용하게 활용할 수 있다.
신규 주문 시스템에서 발생한 주문 데이터를 기존 주문 시스템에 반영해야 하는 요구가 있었다. 이를 위해 신규 주문 시스템에서 주문이 생성되거나 변경되면 그 데이터를 기존 주문 시스템에 전달해야했다.

```mermaid
---
title: 신규 주문 시스템의 코드를 수정하지 않고 CDC를 사용해서 변경 데이터를 관련 시스템에 전파했다.
---

flowchart LR
    newOrder[신규 주문 시스템]
    newOrder --주문 데이터 변경--> DB
    DB --변경 통지--> CDC[CDC 처리기]
    CDC --변경 메시지 추가--> messageQueue[메시징 시스템]

    consumer1[소비자 1] --> messageQueue
    consumer2[소비자 2] --> messageQueue
    consumer1 --> oldOrderDB[구주문 DB]
    consumer2 --> oldContentDB[구컨텐츠 DB]
```
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-6.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-7.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-8.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-9.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-10.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-11.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-12.png)
...
![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-13.png)

VPN은 인터넷과 같은 공용 네트워크에서 서로 다른 네트워크 간에 암호화된 연결을 제공한다.
두 네트워크는 마치 하난의 사설 네트워크에 존재하는 것처럼 연결될 수 있다.

![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-14.png)

집이나 카페 같은 외부 공안에서 서버 네트워크에 접근해야 할 때 이 방식을 사용한다. (VPN 클라이언트)

## TCP, UDP, QUIC

TCP가 전송 보장을 하기 위해 사용하느 시퀀스 번호, 확인 응답, 재전송 등이 추가되면서 전송 속도는 UDP보다 느리다.
예를 들어, 일부 패킷이 유실되면 해당 패킷이 도착할 때까지 이후 패킷을 제대로 처리하지 못하는 HOL 블로킹 (Head-of-Line Blocking) 문제가 있는데 이는 전체적인 전송 속도를 느리게 만든다.

TCP는 신뢰성이 있지만 느리다. 반면에 UDP는 빠르지만 신뢰성이 없다.
이 둘을 합쳐 놓은 것이 QUIC 프로토콜이다.

QUIC는 UDP를 기반으로 하며, TCP의 연결 관리 기능을 QUIC 프로토콜 수준에서 제공한다.
예를 들어 QUIC는 데이터에 연결 ID를 포함시키는데, 이 연결 ID를 이용해서 두 노드 간의 연결을 유지한다.
또한 TCP의 혼잡 제어나 패킷 유실 복구와 같은 기능을 QUIC 프로토콜에서 제공한다.

QUIC는 TLS(Transport Layer Security)를 통합했다.
QUIC 패킷은 기본적으로 암호화되어 전송된다.
또한 TCP 기준의 HTTPS는 TCP 연결을 위한 3-way Handshake와 TLS 연결을 위한 TLS Handshake를 각각 수행해야 한다.
QUIC는 TLS Handshake를 TCP Handshake와 통합해서 연결을 위한 핸드쉐이크를 1회만 수행한다.

![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-15.png)

QUIC는 멀티플렉싱을 지원한다.
QUIC은 하나의 연결에서 여러 스트림을 동시에 처리할 수 있다.
여러 스트림을 동시에 처리하므로 1개의 스트림에서 HOL 블로킹이 발생해도 다른 스트림에는 영향을 주지 않는다.
참고로 HTTP/2도 멀티플렉싱을 지원하지만 TCP 자체에는 발생하는 HOL 블로킹 문제를 피할 수는 없다.
...
하위 계층은 상위 계층에 대한 의존을 갖지 않는다. 오직 상위 계층에서 하위 계층으로의 의존만 허용한다.

![alt text](/images/주니어-백엔드-개발자가-반드시-알아야-할-실무-지식/image-16.png)

표현 계층은 사용자의 요청을 처리하고, 사용자에게 응답을 반환하는 역할을 한다.
응용 계층은 모델 계층이나 인프라 계층을 사용해서 필요한 기능을 구현하고 결과를 표현 계층에 리턴한다.
흔히 서비스라는 기능이 응용 계층에 해당한다.

도메인/모델 계층은 도메인 로직을 구현한다. 주문 모델의 취소 제약 조건과 상태 변경과 같은
로직이 이 계층에 속한다. 인프라/영속 계층은 DB 연동이나 문자 발송과 같은 구현 기술을 구현한다.

### 흩어지는 도메인 로직

단순히 서비스/DAO로 구성된 계층형 아키텍처를 사용하면 실제로 계층은 3개가 된다.
도메인 영역이 (거의) 없는 계층 구조에서 도메인 로직이 인프라와 응용 계층으로 분산되는 경향이 있어
코드 유지보수를 어렵게 만들기도 한다. 도메인 계층이 없으면 도메인 로직이 각 계층으로 펴지는 경향을 보인다.

## DDD와 전술 패턴

| 구성 요소     | 설명                                                                                                                                                                                                                      |
| :------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 도메인        | 각 엔티티 객체는 고유의 식별자를 가지며, 각 엔티티는 식별자로 구분된다(equals & hashcode). 내부 상태가 바뀌어도 식별자는 바뀌지 않는다.예를 들어 주문 엔티니는 서로 다른 주문번호를 식별자로 갖는다.                      |
| 밸류          | 밸류는 고유의 식별자를 갖지 않으며 개념적인 값을 표현한다. 금액, 배송 주소 같은 값이 벨류가 된다. 값은 불변으로 구현하는 것을 추천한다.                                                                                   |
| 애그리거트    | 애그리거트는 관련된 객체를 묶어 하나의 개념적인 단위를 표현한다. 예를 들어 주문 애그리거트는 Order 엔티티, OrderLine 밸류 집합, ShippingAddress 밸류로 구성될 수 있다. 애그리거트는 모델의 일관성을 관리하는 단위가 된다. |
| 리포지토리    | 도메인 객체를 물리적인 저장소와 연결할 때 사용되는 모델이 리포지토리이다. 리포지토리는 도메인 객체를 저장하고 조회할 때 사용되는 인터페이스를 제공한다. **리포지토리는 애그리거트 단위로 존재한다.**                      |
| 도메인 서비스 | 도특정한 애그리거트에 속하지 않은 로직을 구현한다. 외부 연동이 필요한 도메인 로직도 도메인 서비스로 사용해서 표현한다.                                                                                                    |
| 도메인 이벤트 | 도메인 내에서 발생한 이벤트를 표현한다. 도메인의 상태가 변경될 때 도메인 이벤트가 발생한다. 도메인 이벤트를 주로 다른 부분에 변화를 알리기 위해 사용된다.                                                                 |

복잡한 모델을 애그리거트 단위로 관리할 수 있게 함으로써 복잡도를 낮추고, 애그리거트에 관련 로직을 모아 응집도를 높인다.
이는 복잡한 도메인의 유지보수성을 높여준다.

```java
/// DDD 전술 패턴은 도메인 로직을 한 곳에 모으는데 도움이 된다.
...
전술패턴 외에도 바운디드 컨텍스트(bounded context)는 도메인 간의 경계를 설정해준다.
이는 상위 수준에서 복잡한 도메인을 관리하는데 도움을 주며, 이어서 살펴볼 마이크로서비스 아키텍처와도 잘 어올린다.

## 마이크로서비스 아키텍처

|      | 모놀리식                                                                                                                                                                                        | 마이크로서비스                                                                                                                                                                           |
| :--- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 장점 | - 배포가 단순하다. <br/> - 코드 관리가 더 쉽다. <br/> - 성능을 높이기 위해 복잡한 구조를 가질 필요가 없다. <br/> - 테스트와 디버깅이 쉽다. <br/>                                                | - 독립적인 배포와 지속적인 배포가 용이하다. <br/> - 성능 확장이 용이하다. <br/> - 기술에 대한 유연성을 가질 수 있다. <br/> - (보통) 개발자의 만족도가 더 높다.                           |
| 단점 | - 규모가 커질수록 개발 속도가 느려질 수 있다. <br/> - 한 기능의 문제가 전체에 영향을 줄 수 있다. <br/> - 구현 기술 변경에 어려움이 있다. <br/> - 작은 변경에도 전체를 다시 배포해야 한다. <br/> | - 테스트와 디버깅이 어려울 수 있다. <br/> - 모놀리식 대비 인프라가 복잡해진다. <br/> - 소통에 따른 부하가 증가할 수 있다. <br/> - 무분별하게 서비스를 만들면 분산 모놀리식이 될 수 있따. |

마이크로서비스 6가지 핵심 개념은 다음과 같다.

1. 독립적 배포: 다른 마이크로 서비스를 배포하지 않고도 마이크로서비스를 변경, 배포, 출시할 수 있어야 한다.
2. 도메인 중심으로 모델링: 각 마이크로 서비스는 도메인을 기준으로 구분해야 한다.
3. 자신의 상태를 가짐: 마이크로서비스는 DB를 공유하지 않는다. 다른 마이크로서비스의 데이터를 사용할 경우 DB에 직접 접근하지 않고 API 등을 통해 접근한다.
4. 크기: 마이크로서비스의 크기는 절대적인 기준은 없다. 마이크로서비스의 크기를 정할 때는 조직이 감당할 수 있는 수준과 마이크로서비스 경계 정의에 집중해야 한다.
...
이벤트는 변경된 데이터를 다른 시스템에 전달할 때 주로 사용된다.

> [!NOTE]  
> **이벤트 브로커와 단일 진실 공급원**  
> 이벤트 브로커를 단일 진실 공급원(Single Source of Truth)으로 확장해서 사용하는 방식도 있다.
> 예를 들어, 카프카는 메시지를 삭제하지 않고 저장할 수 있는데, 이 특징을 활용하면
> 이벤트 생산자가 보낸 모든 이벤트를 영구 보관할 수 있다.
> 시스템의 모든 상태 변화가 이벤트로 발생한다면, 이벤트 기록만으로도 시스템 상태의 변화를 추적할 수 있다. 더 나아가 카프카 같은 브로커를 이벤트 소싱 패턴의 이벤트 저장소로도 활용할 수 있다.
...
예를 들어 주문을 생각해보면, 주문을 생성할 때는 주문 ID, 회원 ID, 배송지 주소, 상품 ID와 같은 데이터를 사용한다.
반면에 주문을 조회할 때는 회원 이름, 상품명과 같은 데이터를 추가로 사용한다.
명령과 조회에서 사용하는 데이터가 서로 다른 것이다.

명령을 위한 모델과 조회를 위한 모델을 분리하는 패턴이다. 여기서 명령은 시스템의 상태를 변경하는 것을, 조회는 상태를 조회하는 것을 의미한다.
